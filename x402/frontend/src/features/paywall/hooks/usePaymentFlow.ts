...
async function fetchPaymentRequirementsFromResource(
  resourceUrl: string,
...
export function usePaymentFlow({
  ...
  const [requirementsSource, setRequirementsSource] = useState<PaymentSource>(
    getWindowRequirements,
  )
  ...
  const fetchRequirements = useCallback(async () => {
    ...
    try {
      ...
      setState('error')
      setStatusMessage(
        'Could not retrieve payment requirements from the server.',
      )
    }
  }, [resourceUrl])
  ...
  useEffect(() => {
    ...
    const unlockContent = async () => {
      ...
      try {
        ...
        setState('error')
        setStatusMessage(
          error instanceof Error ? error.message : 'Unable to unlock content',
        )
      }
    }
