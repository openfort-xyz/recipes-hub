...
- Lets a Shield-managed embedded wallet discover, enter, track, and exit native MON staking on Monad Testnet via the Yield.xyz v1 REST API (`monad-testnet-mon-native-staking`).
...
- Full enter/exit verification needs a funded testnet wallet - get MON from [testnet.monad.xyz](https://testnet.monad.xyz/).
...
- Solana staking on Yield.xyz is **mainnet-only** (no `solana-devnet` yield opportunities) - that's why this recipe targets Monad Testnet instead of Solana.
...
4. **Add a row to the root `README.md`**, in both tables:
   - Recipes table: `| **[MON Staking on Yield.xyz](./yield-xyz/)** | Native MON staking on Monad Testnet via Yield.xyz's StakeKit API - discover, enter, track, and exit, entirely non-custodial. No other Yield.xyz partner recipe has a working Monad demo. | \`pnpx gitpick openfort-xyz/recipes-hub/tree/main/yield-xyz openfort-yield-xyz && cd openfort-yield-xyz\` |`
   - Stack Overview table: `| **Yield.xyz** | Vite + React | Express.js (openfort-backend-quickstart) | Monad Testnet | \`wagmi\`, \`viem\`, hand-rolled REST client |`
