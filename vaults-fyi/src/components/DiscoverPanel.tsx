...
export function DiscoverPanel({
  ...
  return (
    ...
    >
      {!userAddress && <p className="text-sm text-neutral-500">Sign in to load recommendations.</p>}
      {isLoading && <p className="text-sm text-neutral-500">Loading…</p>}
      {error && <p className="text-sm text-red-400">{(error as Error).message}</p>}
      {data?.length === 0 && <p className="text-sm text-neutral-500">No vaults found.</p>}
        ...
        {data?.slice(0, 5).map((vault) => (
          ...
          >
            <div className="text-left">
              ...
              <div className="text-xs text-neutral-500 truncate max-w-[280px]">{vault.address}</div>
            </div>
            <div className="text-sm text-emerald-400 font-semibold">{(vault.apy['7day'].total * 100).toFixed(2)}%</div>
          </button>
        ))}
      </div>
    </Card>
