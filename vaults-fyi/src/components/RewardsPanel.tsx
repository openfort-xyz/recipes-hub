...
export function RewardsPanel({ userAddress }: { userAddress: string }) {
  ...
  const totalUsd = networkRewards.reduce((sum, r) => sum + parseFloat(r.asset.claimableAmountInUsd ?? '0'), 0)
  ...
  return (
    ...
    >
      ...
      {!isLoading && networkRewards.length === 0 && <p className="text-sm text-neutral-500">No claimable rewards.</p>}
          ...
          <ul className="space-y-1 text-sm text-neutral-300 mb-4">
            {networkRewards.map((r) => (
              <li key={r.claimId}>
                ...
                {r.asset.claimableAmountInUsd ? ` (${r.asset.claimableAmountInUsd} USD)` : ''}
                {' · '}
                <span className="text-neutral-500">{r.sources.map((s) => s.protocol.name).join(', ')}</span>
              </li>
            ))}
          </ul>
          <button
