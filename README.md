...
| **[MPP Agent](./mpp/)**                                      | Backend-wallet MPP (Machine Payments Protocol) agent. An AI agent funds itself with PathUSD and autonomously pays for an HTTP `402` service, settling on the Tempo testnet — Openfort signs every payment.  | `pnpx gitpick openfort-xyz/recipes-hub/tree/main/mpp openfort-mpp && cd openfort-mpp`                      |
...
| **MPP**                 | Next.js 16   | Next.js    | Tempo Testnet     | `@openfort/openfort-node`, `mppx`, `ox`, `viem`     |
