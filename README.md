...
| **[Gnosis Pay card](./gnosis-pay/)**                         | Own a self-custodial Gnosis Pay account from an embedded wallet. Deploy the Safe, set up its Roles + Delay modules and spending allowance, and fund it with EURe on Gnosis Chain via account-kit. | `pnpx gitpick openfort-xyz/recipes-hub/tree/main/gnosis-pay openfort-gnosis-pay && cd openfort-gnosis-pay`   |
...
| **Gnosis Pay**          | React Native | -          | Gnosis Chain      | `@gnosispay/account-kit`, `@openfort/react-native`, `ethers` |
