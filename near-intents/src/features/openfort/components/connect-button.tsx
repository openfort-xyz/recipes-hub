...
import { BLOCKCHAIN_LABEL } from "@/features/near-intents/constants";
import { OpenfortButton, useSignOut } from "@openfort/react";
import { Button } from "@/components/ui/button";

const CHAIN_NAME: Record<number, string> = {
  1: BLOCKCHAIN_LABEL.eth,
  8453: BLOCKCHAIN_LABEL.base,
  42161: BLOCKCHAIN_LABEL.arb,
  10: BLOCKCHAIN_LABEL.op,
  137: BLOCKCHAIN_LABEL.pol,
  43114: BLOCKCHAIN_LABEL.avax,
};
...
export default function OpenfortConnectButton({
  ...
  const { isConnected, isAuthenticated, playerName, address, chainId } =
    useOpenfortWallet();
    ...
    <div className={cn("flex items-center gap-3", className)}>
      <div className="flex flex-col gap-1 text-sm">
        ...
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          <span>{chainId ? CHAIN_NAME[chainId] ?? `Chain ${chainId}` : "—"}</span>
        </div>
      </div>
