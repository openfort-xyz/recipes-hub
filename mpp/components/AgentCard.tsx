...
export function AgentCard({ agent }: AgentCardProps) {
  ...
  const fetchBalance = useCallback(async () => {
    try {
      const res = await fetch(`/api/agent/balance?address=${agent.wallet.address}`)
      const data = await res.json()
      if (data.success) {
        setOnChainBalance(data.balance)
      }
    } catch {
      // Silently fall back to the client-side balance.
    }
      ...
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-2">
            <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#FC3627]">
              <svg
        ...
        <div className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-medium text-emerald-700">Active</div>
      </div>

      <div className="mt-6 space-y-4">
        <div className="rounded-xl bg-[#F1F2F9] p-4">
          <div className="text-xs font-medium uppercase tracking-wide text-slate-500">Wallet Address</div>
          <div className="mt-1 font-mono text-sm text-slate-900">{truncatedAddress}</div>
        </div>

        <div className="rounded-xl bg-[#F1F2F9] p-4">
          <div className="text-xs font-medium uppercase tracking-wide text-slate-500">Balance</div>
          <div className="mt-1 text-xl font-semibold text-slate-900">{formatUsd(displayBalance)}</div>
