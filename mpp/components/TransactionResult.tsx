    ...
    >
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          <h3 className={`font-semibold ${isSuccess ? 'text-emerald-900' : 'text-red-900'}`}>
            {isSuccess ? 'Transaction successful!' : 'Transaction failed'}
          </h3>

          <p className={`mt-1 text-sm ${isSuccess ? 'text-emerald-700' : 'text-red-700'}`}>
            {isSuccess
            ...
            <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-100 p-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-emerald-800">Payment signed via Openfort</span>
                </div>
                <a
                  href={`${result.explorerUrl || 'https://explore.tempo.xyz'}/address/${result.walletAddress}`}
                  target="_blank"
          ...
          {isSuccess && result.result?.data && (
            <div className="mt-4 rounded-xl bg-white p-4 shadow-sm">
              <div className="text-xs font-medium uppercase tracking-wide text-slate-500">Response Data</div>
