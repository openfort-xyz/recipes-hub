...
export default function FundStep() {
  ...
  const handleFund = async () => {
    ...
    try {
      ...
      const updatedAgent = { ...agent, balance: agent.balance + selectedAmount * 100 }
    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-2xl px-6 py-12">
        ...
        <div className="text-center">
          <div className="mx-auto mb-6 flex h-16 w-16 items-center justify-center rounded-2xl bg-[#FC3627]">
            <svg
          ...
          <p className="mt-3 text-slate-600">Add PathUSD to your agent&apos;s wallet so it can make payments</p>
        </div>

        {/* Wallet info */}
        <div className="mt-8 rounded-2xl border border-[#E2E3F0] bg-white p-6">
          <div className="flex items-center gap-4">
            <div className="flex h-12 w-12 items-center justify-center rounded-full bg-[#FC3627]">
              <svg
        ...
        {/* Funding options */}
        <div className="mt-8">
          <span className="mb-4 block text-sm font-medium text-slate-700">Select funding amount</span>
              ...
              <button
                ...
                className={`relative rounded-2xl border-2 p-6 text-center transition-all ${
                  selectedAmount === option.amount
                    ? 'border-[#FC3627] bg-[#FFE7E4] shadow-lg'
                    : 'border-[#E2E3F0] bg-white hover:border-gray-300 hover:shadow-md'
                }`}
              >
                {option.description === 'Recommended' && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-[#FC3627] px-3 py-1 text-xs font-semibold text-white">
                    Popular
                  </div>
                )}
        ...
        <div className="mt-8 flex gap-4">
          ...
          <button
            ...
            className="flex-1 rounded-xl bg-[#FC3627] px-6 py-4 text-sm font-medium text-white transition-all hover:bg-[#E02D1F] disabled:cursor-not-allowed disabled:opacity-50"
          >
            {isLoading ? (
