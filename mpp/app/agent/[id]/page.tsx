...
export default function ActionsStep() {
  ...
  const [transactionResult, setTransactionResult] = useState<TransactionResultData | null>(null)
  ...
  const handleExecuteAction = async (action: 'weather') => {
    ...
    try {
      ...
      if (data.success) {
        const updatedAgent = { ...agent, balance: agent.balance - ACTION_COSTS[action] }
  ...
  if (!agent) {
    return (
      <div className="flex min-h-screen items-center justify-center">
        <div className="h-8 w-8 animate-spin rounded-full border-4 border-[#E2E3F0] border-t-[#FC3627]" />
      </div>
    )
  }
    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-4xl px-6 py-8">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-slate-600">Execute actions and watch your agent make autonomous payments</p>
        </div>
        ...
        <div className="space-y-6">
          ...
          {/* Transaction result */}
          {transactionResult && (
            <TransactionResult result={transactionResult} onDismiss={() => setTransactionResult(null)} />
          )}
          ...
          <div className="rounded-2xl border border-[#E2E3F0] bg-white p-6">
            ...
            <div className="space-y-3 text-sm text-slate-600">
              <div className="flex gap-3">
                <div className="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full bg-[#FC3627] text-xs font-bold text-white">
                  1
                </div>
                <p>
                  Your agent requests a paid resource. The service responds with HTTP{' '}
                  <strong>402 Payment Required</strong> and payment details.
                </p>
              </div>
              <div className="flex gap-3">
                <div className="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full bg-[#FC3627] text-xs font-bold text-white">
                ...
                <p>
                  ...
                  automatically signs a payment credential using the agent&apos;s Openfort wallet and retries the
                  request.
                </p>
              </div>
              <div className="flex gap-3">
                ...
                <p>
                  The service verifies the credential and settles payment on <strong>Tempo</strong> using{' '}
                  <strong>PathUSD</strong>, then returns the requested data.
                </p>
              </div>
            </div>
          </div>
