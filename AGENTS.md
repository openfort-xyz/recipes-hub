...
- **Match the `wagmi` major to the `@openfort/react` version.** `@openfort/react` `>=1.1.1` peer-requires `wagmi: 3.x` (`@wagmi/connectors: 8.x`); `<=1.1.0` requires `wagmi: 2.x` (`@wagmi/connectors: 5.x`). Keep `viem` on `^2.x` either way. The Vite-based samples (`aave`, `morpho`, `vaults-fyi`, `x402`) run `@openfort/react@^1.1.1` + `wagmi@^3`; the Next.js samples (`7702`, `lifi`, `near-intents`) are still on `wagmi@^2` until bumped.
