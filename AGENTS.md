...
## Project overview
- Collection of Openfort integration samples (`7702`, `aave`, `agent-permissions`, `hyperliquid`, `lifi`, `lighter`, `morpho`, `mpp`, `near-intents`, `private-payments`, `telegram-bot`, `usdc`, `vaults-fyi`, `x402`, `yield-xyz`, `zama-confidential-yield`).
...
- **Match the `wagmi` major to the `@openfort/react` version.** All web samples that use `@openfort/react` (`7702`, `aave`, `agent-permissions`, `lifi`, `morpho`, `near-intents`, `vaults-fyi`, `x402`) run the latest published `@openfort/react` (pinned exactly, same version in every recipe) + `wagmi@^3`; keep `viem` on `^2.x`.
